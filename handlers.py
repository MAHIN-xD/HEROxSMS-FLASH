...
def extract_otp_code(text_or_code: str) -> str:
    """এসএমএস টেক্সট বা স্ট্রিং থেকে ওটিপি কোড বের করে আনার মেথড"""
    if not text_or_code:
        return ""
...
def format_otp_text(phone: str) -> str:
    """স্ক্রিনশটের মতো নিখুঁত মেসেজ: 🇨🇴 Telegram 573204702246"""
    clean_phone = str(phone).lstrip("+").strip()
    safe_phone = html.escape(clean_phone)
    return f"🇨🇴 <b>Telegram</b> {safe_phone}"

# --- পিওর ওয়েবhooks হ্যান্ডলার (১০০% নিখুঁত ওটিপি ক্যাচিং ও প্রিন্ট) ---
async def handle_herosms_webhook(request: web.Request):
    data = {}
    # ১. GET কোয়েরি প্যারামিটার বা POST বডি থেকে ডাটা রিড করা
    try:
    ...
    # ২. HeroSMS ফিল্ডস এক্সট্র্যাক্ট করা
    ...
    # ৩. ডাটাবেজ থেকে ইউজার ও ফোন নম্বর ম্যাচ করা
    try:
        user_id = None
        phone = direct_phone
        ...
        # ৪. টেলিগ্রামে ওটিপি মেসেজ ও বাটন পাঠানো
...
# ম্যানুয়াল চেক বাটনে চাপ দিলেও সরাসরি এই ফরম্যাটে মেসেজ আপডেট হবে
...
async def cb_noop(callback: CallbackQuery):
    await callback.answer()DEFAULT_OPERATOR = "any"
